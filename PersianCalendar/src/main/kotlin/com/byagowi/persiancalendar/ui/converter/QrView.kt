...
fun QrView(text: String, setShareAction: (() -> Unit) -> Unit) {
    ...
    fun drawQr(canvas: Canvas, size: Float) {
        ...
        path.rewind()
        run {
            val r = CornerRadius(cellSize * roundness * 2)
            path.addRoundRect(
                RoundRect(0f, 0f, cellSize * 7, cellSize * 7, r, r, r, r),
                Path.Direction.Clockwise,
            )
        }
        run {
            val r = CornerRadius(cellSize * roundness * 1.5f)
            path.addRoundRect(
                RoundRect(cellSize, cellSize, cellSize * 6, cellSize * 6, r, r, r, r),
                Path.Direction.CounterClockwise,
            )
        }
        run {
            val r = CornerRadius(cellSize * roundness)
            path.addRoundRect(
                RoundRect(cellSize * 2, cellSize * 2, cellSize * 5, cellSize * 5, r, r, r, r),
                Path.Direction.Clockwise,
            )
        }
