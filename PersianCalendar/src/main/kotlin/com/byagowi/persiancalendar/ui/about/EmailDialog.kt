...
fun NoteOnAppointments(onDismissRequest: () -> Unit, nextStep: (() -> Unit)? = null) {
    CompositionLocalProvider(
        ...
        AppDialog(
            ...
            Text(
                """به دلیل محدودیت‌هایی از جمله در حجم، آفلاین‌بودن، اختلاف نظر در منابع و احتمال عدم تطابق با رویدادها در زمان رخ‌دادن آن‌ها در این برناهه همواره هدف اصلی نمایش صحیح تعطیلی‌های رسمی به دلیل اهمیت بنیادین آن‌ها در برنامه‌ریزی کارها در زندگی واقعی بوده است. در نسخه‌های سال‌ها پیش این برنامه مناسبت‌های غیرتعطیل قرار نداده شده بود که بعدها توسط سایر توسعه‌دهندگان به پروژه اضافه شدند که به همین دلیل ناقص بوده و بصورت پیش‌فرض غیرفعال است.
