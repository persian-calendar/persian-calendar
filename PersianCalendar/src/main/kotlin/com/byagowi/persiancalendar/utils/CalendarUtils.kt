...
val PersianDate.isOldEra get() = year < 1304 && year != EventsRepository.EVERY_YEAR
...
fun Date.toGregorianCalendar(forceLocalTime: Boolean = false): GregorianCalendar {
    ...
    if (!forceLocalTime && isForcedIranTimeEnabled) calendar.timeZone =
        TimeZone.getTimeZone(IRAN_TIMEZONE_ID)
    calendar.time = this
    return calendar
}

fun GregorianCalendar.formatDateAndTime(
    ...
    return language.timeAndDateFormat.format(
        Clock(this).toFormattedString(withSeconds = withSeconds),
